...
export class GFGService extends BaseProviderService {
	...
	async scrapeDailyQuestion(): Promise<Problem> {
		try {
			...
			await this.safeGoto(
				`${this.BASE_URL}/problem-of-the-day`,
				this.BASE_URL,
			);
			...
			try {
				...
				console.warn(
					'Element with id="potd_solve_prob" not found, trying fallback method',
				);
				try {
					problemUrlRelative = await this.page.evaluate(() => {
						...
						for (const link of links) {
							...
							if (
								button &&
								button.textContent?.includes("Solve Problem")
							) {
						...
						const mainContent =
							document.querySelector(
								'div[class*="problemOfTheDay"]',
							) ||
							document.querySelector("main") ||
							document.body;
						if (mainContent) {
							const firstProblemLink = mainContent.querySelector(
								'a[href*="/problems/"]',
							);
							return firstProblemLink
								? firstProblemLink.getAttribute("href")
								: null;
						}
						return null;
					});
			...
			// Construct full URL
			const problemUrl = this.validateUrl(
				problemUrlRelative,
				this.BASE_URL,
			);
			...
			await this.waitForElement(
				'div[class*="problems_header_content"] h3, div[class*="header_content"] h3',
				10000,
			);
			...
			try {
				const difficultyText = await this.page.evaluate(() => {
					const descDiv = document.querySelector(
						'div[class*="problems_header_description"], div[class*="header_description"]',
					);
			...
			const fullDescription = await this.page.evaluate(() => {
				const contentDiv = document.querySelector(
					'div[class*="problems_problem_content"], div[class*="problem_content"]',
				);
			...
			const isPremium =
				(await this.page.$('[class*="premium"], [class*="locked"]')) !==
				null;
		...
		try {
			const examples = await this.page.evaluate(() => {
				...
				const preElements = Array.from(
					contentDiv.querySelectorAll("pre"),
				);
				...
				preElements.forEach((pre, index) => {
					...
					const inputMatch = preHTML.match(
						/<strong[^>]*>Input[^<]*<\/strong>[\s:]*([\s\S]*?)(?=<strong[^>]*>Output|<strong[^>]*>Explanation|$)/i,
					);
					const outputMatch = preHTML.match(
						/<strong[^>]*>Output[^<]*<\/strong>[\s:]*([\s\S]*?)(?=<strong[^>]*>Explanation|<strong[^>]*>Input|$)/i,
					);
					const explanationMatch = preHTML.match(
						/<strong[^>]*>Explanation[^<]*<\/strong>[\s:]*([\s\S]*?)(?=<strong[^>]*>Input|<strong[^>]*>Output|$)/i,
					);
		...
		try {
			const constraints = await this.page.evaluate(() => {
				...
				for (const p of paragraphs) {
					...
					if (
						strongTag &&
						strongTag.textContent?.includes("Constraints")
					) {
						// Get all spans after the strong tag
						const allSpans = Array.from(p.querySelectorAll("span"));
						const constraintsSpanIndex = allSpans.findIndex(
							(span) =>
								span
									.querySelector("strong")
									?.textContent?.includes("Constraints"),
						);

						if (
							constraintsSpanIndex >= 0 &&
							constraintsSpanIndex < allSpans.length - 1
						) {
							// Get the next span(s) that contain the actual constraints
							for (
								let i = constraintsSpanIndex + 1;
								i < allSpans.length;
								i++
							) {
